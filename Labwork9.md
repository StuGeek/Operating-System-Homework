...
int main(int argc, char *argv[])
{
    ...
    // 用open()函数打开一个FIFO文件，其中fifoname是FIFO文件路径名，O_RDWR代表权限可读可写，默认情况下以阻塞模式进行读写，返回一个FIFO的文件描述符给fdw
    ...
    // 否则打开管道成功
        ...
        // 使用fcntl()函数把文件标识符为fdw的FIFO文件的写设置为非阻塞模式，其中F_SETFL表示设置文件打开方式为flags | O_NONBLOCK方式，即之前方式的基础上加上非阻塞方式
